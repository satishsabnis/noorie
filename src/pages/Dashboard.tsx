...
async function fetchBriefLowStock(salonId: string): Promise<BriefLowStock[]> {
  const { data } = await supabase
    .from('inventory_items')
    .select('name, stock_count, low_stock_threshold, unit')
    .eq('salon_id', salonId)
    .eq('type', 'product')
  if (!data) return []
  return (data as { name: string; stock_count: number; low_stock_threshold: number; unit: string }[])
    .filter(item => item.stock_count <= item.low_stock_threshold)
    .map(item => ({
      name: item.name,
...
function buildLowStockHTML(items: BriefLowStock[], salonName: string, tz: string): string {
  const dateStr = new Date().toLocaleDateString('en-GB', { timeZone: tz, day: 'numeric', month: 'long', year: 'numeric' })
  const rows = items.map(item => {
    const isZero = item.stockCount === 0
    const isLow  = !isZero && item.stockCount <= item.lowStockThreshold
    const color  = isZero ? '#991b1b' : isLow ? '#f97316' : '#000000'
    return `<tr>
      <td style="padding:8px 12px;border-bottom:1px solid #f0f0f0;">${escapeHtml(item.name)}</td>
      <td style="padding:8px 12px;border-bottom:1px solid #f0f0f0;">${escapeHtml(item.unit)}</td>
      <td style="padding:8px 12px;border-bottom:1px solid #f0f0f0;color:${color};font-weight:600;">${item.stockCount}</td>
      <td style="padding:8px 12px;border-bottom:1px solid #f0f0f0;">${item.lowStockThreshold}</td>
    </tr>`
  }).join('')
  return `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Low Stock Report</title>
  <style>body{font-family:sans-serif;margin:40px;color:#111;}h1{color:#034325;font-size:22px;margin-bottom:4px;}
  table{width:100%;border-collapse:collapse;margin-top:16px;}th{background:#034325;color:#fff;padding:8px 12px;text-align:left;font-size:12px;}
  td{font-size:12px;} .footer{margin-top:24px;font-size:10px;color:#9ca3af;}</style></head><body>
  <h1>${escapeHtml(salonName)}</h1>
  <p style="font-size:13px;color:#6b7280;margin:0 0 4px;">Low Stock Report</p>
  <p style="font-size:12px;color:#9ca3af;margin:0 0 16px;">${dateStr}</p>
  <table><thead><tr><th>Product Name</th><th>Unit</th><th>Stock Count</th><th>Threshold</th></tr></thead>
  <tbody>${rows}</tbody></table>
  <p class="footer">Generated by Noorie | Blue Flute Consulting LLC-FZ</p>
  </body></html>`
}
...
function MorningBrief({
  ...
  const [activeModal, setActiveModal] = useState<'slots' | 'lapsed' | 'unpaid' | 'topClient' | 'lowStock' | null>(null)
          ...
          <div style={{ backgroundColor: '#fff', borderRadius: 12, maxWidth: 420, width: '90%', padding: 24, maxHeight: '60vh', overflowY: 'auto' }}>
            ...
            {activeModal === 'lowStock' && (
              loading
                ...
                ? <p style={{ fontSize: 12, color: '#6b7280', margin: 0 }}>Unable to load — check connection.</p>
                : lowStock.length === 0
                ? <p style={{ fontSize: 12, color: '#6b7280', margin: 0 }}>No low stock items.</p>
                : <>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 20 }}>
                      {lowStock.map((item, i) => {
                        const isZero = item.stockCount === 0
                        const stockColor = isZero ? '#991b1b' : '#f97316'
                        return (
                          <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', borderBottom: '0.5px solid #f0f0f0', paddingBottom: 8 }}>
                            <div>
                              <p style={{ margin: '0 0 2px', fontSize: 13, fontWeight: 500, color: '#111' }}>{item.name}</p>
                              <p style={{ margin: 0, fontSize: 11, color: '#6b7280' }}>threshold: {item.lowStockThreshold} · {item.unit}</p>
                            </div>
                            <p style={{ margin: 0, fontSize: 13, fontWeight: 700, color: stockColor }}>{item.stockCount}</p>
                          </div>
                        )
                      })}
                    </div>
                    <button
                      onClick={() => openPrintWindow(buildLowStockHTML(lowStock, salonName ?? 'Salon', tz), `LowStock_${(salonName ?? 'Salon').replace(/\s+/g, '_')}_${new Date().toISOString().slice(0, 10)}`)}
                      style={{ backgroundColor: '#034325', color: '#ffffff', border: 'none', borderRadius: 6, padding: '8px 18px', fontSize: 13, cursor: 'pointer', width: '100%' }}
                    >
                      Download PDF
                    </button>
                  </>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2, 1fr)' : 'repeat(4, 1fr)', gap: 10 }}>
          ...
          {lowStock.length > 0 && (
            <div
              ...
              style={{ ...tileStyle, border: '1px solid rgba(255,165,0,0.4)', gridColumn: isMobile ? 'span 2' : 'span 4' }}
            >
              <p style={{ fontSize: 13, fontWeight: 500, color: '#ffffff', margin: '0 0 4px' }}>LOW STOCK</p>
              <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.65)', margin: 0 }}>Need your attention urgently</p>
            </div>
          )}
          {/* Slot 6: loyalty AI tile */}
        </div>
      </div>
    </>
  )
...
export default function Dashboard() {
  ...
  }, [showProductSales, staffRecord?.salon_id])
  ...
  const psCanNext = psSelectedClient !== null && psCartItems.length > 0
  ...
  useEffect(() => {
    ...
    async function fetchBrief() {
      ...
      const [slotsRes, lapsedRes, unpaidRes, topClientRes, lowStockRes] = await Promise.all([
        ...
        fetchBriefLowStock(salonId).then(d => ({ d, e: false })).catch(() => ({ d: [] as BriefLowStock[], e: true })),
      ])
      if (!cancelled) {
        ...
        setBriefLowStock(lowStockRes.d)
          ...
          <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.45)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ backgroundColor: '#fff', borderRadius: 12, maxWidth: 440, width: '94%', maxHeight: '85vh', display: 'flex', flexDirection: 'column' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 20px', borderBottom: '0.5px solid #f0f0f0', flexShrink: 0 }}>
                ...
                <button onClick={closeProductSales} style={{ background: 'none', border: '0.5px solid #034325', color: '#034325', borderRadius: 6, padding: '4px 12px', fontSize: 12, cursor: 'pointer' }}>Close</button>
              </div>
