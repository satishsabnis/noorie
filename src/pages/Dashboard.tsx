...
export default function Dashboard() {
  ...
  const [psStaffList,      setPsStaffList]      = useState<{ id: string; name: string }[]>([])
  ...
  useEffect(() => {
    ...
    supabase.from('staff').select('id, name').eq('salon_id', salonId)
      ...
      .then(({ data }) => setPsStaffList((data ?? []) as { id: string; name: string }[]))
      ...
      <div style={{ marginTop: 52, flex: 1, display: 'flex', flexDirection: 'column' }}>
        ...
        {showProductSales && (
          <ProductSaleModal
            ...
            staffList={psStaffList}
            onClose={() => setShowProductSales(false)}
            onSuccess={() => { setPsSaleSuccess(true); setTimeout(() => setPsSaleSuccess(false), 3000) }}
          />
        )}
